...
gh api \
  ...
  -f 'reviewers[]=copilot' > /dev/null 2>&1 || true  # ignore 422 if already requested
...
for i in $(seq 1 6); do
  ...
  COUNT=$(gh pr view "$PR" --json reviews \
    --jq "[.reviews[] | select(.author.login | test(\"copilot\"; \"i\")) | select(.submittedAt > \"$START_TIME\")] | length")
  if [ "$COUNT" -gt 0 ]; then
    DETECTED=true
