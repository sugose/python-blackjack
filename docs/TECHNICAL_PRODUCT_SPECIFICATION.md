...
| `GrokProvider` | xAI Grok via their API |
| `OllamaProvider` | Local models via HTTP (no API key required) |
| `MockProvider` | Deterministic responses for testing |

Provider selected via `--provider` CLI flag or `AI_PROVIDER` env var. Provider-specific config (model name, base URL, API key) via env vars. Optional dependencies — graceful error if provider SDK not installed.

### PBI-2.2 — AI Player Strategy

`AIStrategyProvider` wraps any `AIProvider` and implements `Callable[[Hand], str]`. Describes hand state and dealer upcard to the model, parses "hit" or "stand" from response. Pluggable at `Player` construction — zero changes to `play_hand()` or `play_session()`.

### PBI-2.3 — Stochastic Strategy Support

A stochastic strategy is defined by a fixed policy with randomised execution. Seeded for reproducibility in tests.

Built-in stochastic strategies:

| Class | Behaviour |
|---|---|
| `WeightedStrategy` | Per-hand-value hit probability table |
| `NoisyDealerStrategy` | Dealer mirror with configurable error rate |

All implement `Callable[[Hand], str]`.

### PBI-2.4 — AI Viewer Mode

`--ai` flag on `src/viewer.py` opens an interactive query loop. Loads and parses JSONL files as today, serialises events into prompt context, accepts natural language questions, streams responses to stdout. Requires PBI-2.1.

### PBI-2.5 — Tournament Runner

`src/tournament.py` orchestrates multi-table sessions with mixed player types.

**CLI:**
```
python -m src.tournament --players anthropic:claude,openai:gpt-4o,human:adam,bot:dealer-mirror --hands 100 --tables 5
```

Each player backed by a named provider or strategy. All hands logged to JSONL as normal. Requires PBI-2.2 and PBI-2.3.

### PBI-2.6 — Post-Match Analysis

`--analyse` flag on viewer AI mode generates a narrative tournament report from the event log. `--perspective each` generates individual post-match reflections written from each player's point of view. Human and bot players included alongside AI players in the narrative. Requires PBI-2.4 and PBI-2.5.
